...
// newAuthStore resolves the identity source. A nil store (and nil
// closer) means no identity is configured and the gateway runs without
// governance stages.
func newAuthStore(ctx context.Context, cfg config.Config) (auth.Store, *auth.Static, func(), error) {
	switch {
	case cfg.Postgres.DSN != "":
		...
		return auth.NewCachedStore(pg, authPosTTL, authNegTTL), nil, pg.Close, nil
	...
	case cfg.Identity != "":
		...
		return auth.NewCachedStore(staticStore, authPosTTL, authNegTTL), staticStore, func() {}, nil
